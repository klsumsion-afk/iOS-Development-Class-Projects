...
let isOldEnough = checkAge(userInputAge)
print(isOldEnough)
...
func itemToCost(item: String) -> Double? {
    guard let itemCount = stock[item], itemCount > 0 else {
        print("Out of Stock")
        return nil
    }
    return prices[item]
}
let shopping = itemToCost(item: "Donuts")
/*:
